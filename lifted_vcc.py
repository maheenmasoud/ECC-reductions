...
def lifted_simplicial_reduction(graph, uncovered_edges, clique_cover):
    ...
    while uncovered_edges:
        print("Length of uncovered edges:", len(uncovered_edges))
        ...
        # if the common neighbors have no uncovered edges, remove the edge from the uncovered edges
        if  len(uncovered_edges_common_neighbors) == 1:
            #print(f"No uncovered edges other than the current edge {edge}")
            uncovered_edges.remove(edge)

            # append the edge to the clique cover
            clique_cover.append({edge})
            continue
...
if __name__ == "__main__":
    ...
    graph = edges_to_adjacency_list('g7.txt')
    profile_main(graph)
